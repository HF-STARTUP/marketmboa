...
class AddonActivationController extends BaseController
{
    ...
    public function activateFromList(AddonPurchaseCodeRequest $request): Redirector|RedirectResponse|Application|JsonResponse
    {
        ...
        if ($data['status']) {
            ...
            $this->addonService->runAddonMigrations($request['path']);

            if (($fullData['name'] ?? '') === 'Builder') {
                $bundle = $this->addonService->activateBuilderBundle();
                if (!$bundle['ok']) {
                    $fullData['is_published'] = 0;
                    file_put_contents($infoPath, "<?php return " . var_export($fullData, true) . ";");
                    if ($request->ajax()) {
                        return response()->json(['status' => 0, 'message' => $bundle['message']]);
                    }
                    ToastMagic::error($bundle['message']);
                    return back();
                }
            }
