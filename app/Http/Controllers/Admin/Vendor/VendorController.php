...
class VendorController extends BaseController
{
    ...
    {
        // Show the per-vendor Builder storefront on/off only when the Builder
        // module exists and is published, and the vendor actually has a
        // storefront (a tenant_domain_configs row keyed by tenant_id = seller id).
        // Query the table directly (never the Builder model class): once the
        // Builder module is deleted its class file is gone but the composer
        // classmap still points at it, so class_exists()/the model would try to
        // include a missing file and fatal. Schema::hasTable stays safe.
        $builderPublished = getCheckAddonPublishedStatus('Builder') === 1;
        $builderConfig = null;
        if ($builderPublished && Schema::hasTable('tenant_domain_configs')) {
            $builderConfig = DB::table('tenant_domain_configs')->where('tenant_id', $seller->id)->first();
        }

        return view('admin-views.vendor.view.setting', compact('seller', 'builderPublished', 'builderConfig'));
    }
    ...
    public function updateSetting(Request $request, $id): RedirectResponse
    {
        ...
        if ($request->has('builder_status_update') && Schema::hasTable('tenant_domain_configs')) {
            $builderAccess = $request['builder_status'] ?? 0;
            if (DB::table('tenant_domain_configs')->where('tenant_id', $id)->exists()) {
                DB::table('tenant_domain_configs')->where('tenant_id', $id)
                    ->update(['website_visibility' => $builderAccess, 'updated_at' => now()]);
            } else {
                DB::table('tenant_domain_configs')->insert([
                    'tenant_id' => $id,
                    'sub_tenant_id' => $this->vendorRepo->getFirstWhere(params: ['id' => $id])?->shop?->id,
                    'website_visibility' => $builderAccess,
                    'created_at' => now(),
                    'updated_at' => now(),
                ]);
            }
            ToastMagic::success(translate('vendor_builder_storefront_access_updated'));
        }
