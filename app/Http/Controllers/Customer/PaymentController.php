...
class PaymentController extends Controller
{
    ...
    public function getCustomerPaymentRequest(Request $request, $orderAdditionalData = []): mixed
    {
        ...
        if (in_array($request['payment_request_from'], ['app'])) {
            ...
            // A pure guest (no account-creation branch) leaves $getCustomerID null,
            // which would persist a null customer on the deferred digital order and
            // break the guest shipping-address handoff/cart clear. Fall back to the
            // guest_users id so the success hook attributes the order to the guest.
