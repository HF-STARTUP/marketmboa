...
class DeliveryManController extends Controller
{
    ...
    public function update_order_status(Request $request):JsonResponse
    {
        ...
        $orderDetailsRewards = OrderDetailsRewards::where(['order_id' => $order->id, 'reward_type' => 'loyalty_point'])->first();

        if ($orderDetailsRewards && $orderDetailsRewards['reward_delivered'] != 1 && $orderDetailsRewards['reward_amount'] > 0 && $loyaltyPointStatus == 1 && $loyaltyPointEachOrder == 1 && !$order->is_guest && $request['status'] == 'delivered') {
            ...
            $orderDetailsRewards->update(['reward_delivered' => 1]);
        }
