...
class OrderController extends Controller
{
    ...
    public function details(Request $request, $id): JsonResponse
    {
        ...
        $deliveryPartnerAvailable = deliveryPartnerAvailable(order: $firstDetails?->order, panel: 'vendor');
        $courierShipment = $this->deliveryPartnerShipmentPayload($firstDetails?->order);

        foreach ($detailsList as $detail) {
            $detail['delivery_partner_available'] = $deliveryPartnerAvailable;
            $detail['courier_shipment'] = $courierShipment;
    ...
    private function deliveryPartnerShipmentPayload(?Order $order): ?array
    {
        if ($order === null) {
            return null;
        }

        $shipment = deliveryPartnerShipmentDetails($order->id);
        ...
        return [
            ...
            'consignment_id' => null,
            'tracking_code' => $order->third_party_delivery_tracking_id,
    ...
    public function assign_third_party_delivery(): JsonResponse
    {
        return response()->json([
            'success' => 0,
            'message' => translate('Manual_third_party_delivery_assignment_is_no_longer_available_Send_the_order_to_a_delivery_partner_instead'),
        ], 403);
    }
    ...
    public function updateOrderDetails(Request $request): JsonResponse
    {
        ...
        if (isset($order)) {
            ...
            if ($request['delivery_type'] == 'third_party_delivery') {
                if (deliveryPartnerShipment($request['order_id']) === null) {
                    return response()->json([
                        'success' => 0,
                        'message' => translate('Manual_third_party_delivery_assignment_is_no_longer_available_Send_the_order_to_a_delivery_partner_instead'),
                    ], 403);
