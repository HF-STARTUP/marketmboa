...
class ForgotPasswordController extends BaseController
{
    ...
    public function getPasswordResetRequest(PasswordResetRequest $request): JsonResponse|RedirectResponse
    {
        ...
        if ($verificationBy == 'email') {
            ...
            if (isset($vendor)) {
                ...
                $this->passwordResetRepo->updateOrCreate(
                    params: ['identity' => $request['identity'], 'user_type' => 'seller'],
                    value: [
                        'token' => $token,
                        'otp_hit_count' => 0,
                        'is_temp_blocked' => 0,
    ...
    public function submitOTPVerificationCode(Request $request): RedirectResponse
    {
        ...
        $otpCode = is_array($request['token']) ? implode('', $request['token']) : $request['token'];
        $verificationData = $this->passwordResetRepo->getFirstWhere(params: ['user_type' => 'seller', 'identity' => $identity]);
        $OTPVerificationData = $this->passwordResetRepo->getFirstWhere(params: ['user_type' => 'seller', 'identity' => $identity, 'token' => $otpCode]);

        $tokenVerifyStatus = false;
        $firebaseOTPVerification = getWebConfig(name: 'firebase_otp_verification') ?? [];
        if ($firebaseOTPVerification && $firebaseOTPVerification['status']) {
            $firebaseVerify = $this->firebaseService->verifyOtp($verificationData['token'], $verificationData['identity'], $otpCode);
