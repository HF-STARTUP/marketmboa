...
class StockClearanceProduct extends Model
{
    ...
    protected static function boot(): void
    {
        ...
        // The Builder storefront does NOT honour Clearance Sale. When a
        // storefront request is in flight (StorefrontContext has a resolved shop
        // scope, set by the storefront middleware before any product query),
        // hide every clearance row so product cards/details, cart, checkout and
        // orders fall back to the regular / product-discount price. Legacy web,
        // admin, vendor panel and mobile API requests never set that scope, so
        // they are unaffected. Guarded by bound() so it no-ops if the Builder
        // module isn't installed.
        static::addGlobalScope('storefront_disable_clearance', function ($builder) {
            $contextClass = \Modules\Builder\Services\StorefrontContext::class;
            if (app()->bound($contextClass) && app($contextClass)->hasScope()) {
