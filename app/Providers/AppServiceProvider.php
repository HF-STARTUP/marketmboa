...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        // Vite (@vite) is only used by the Builder storefront, so only adjust its
        // asset paths when Builder exists and is published. Under a root docroot,
        // public/ is served as a subfolder, so those URLs need the public/ prefix
        // — mirrors dynamicAsset(). A public docroot serves from public/ directly,
        // so no prefix. Lets the storefront load assets in both layouts without a
        // project-root build/ copy or symlink.
        ...
        if (!App::runningInConsole()) {
            ...
            try {
                $hasBusinessSettings = $this->schemaHasTable('business_settings');
                $hasPublishingHouses = $this->schemaHasTable('publishing_houses');
                $hasAuthors = $this->schemaHasTable('authors');
                ...
                if ($hasBusinessSettings) {
                    ...
                    // Drive the Builder storefront currency capability from the
                    // host's `currency_model` business setting. Overridden here
                    // via Config::set (host-side) so the vendor Builder module
                    // stays untouched — it just reads config('builder.capabilities').
                    $multiCurrency = getWebConfig(name: 'currency_model') === 'multi_currency';
                    Config::set('builder.capabilities.currency', [
                        'mode'     => $multiCurrency ? 'multi' : 'single',
                        'switcher' => $multiCurrency,
                    ]);

                    // Drive the Builder storefront map capability from whether the
                    // host has a Google Maps API key configured. No key → the
                    // storefront hides the map picker (navbar + checkout address
                    // form) and falls back to manual address entry.
                    Config::set('builder.capabilities.location.map', !empty(getWebConfig(name: 'map_api_key')));
    ...
    /**
     * Existence check for a permanent table, caching ONLY positive results and
     * only briefly. A missing table at boot is always transient in production
     * (DB blip / connection cap / migration mid-run), so a false result is
     * never cached — the next request re-checks and self-heals. Once the table
     * exists it stays, so a short positive cache still spares information_schema
     * on every request.
     */
    protected function schemaHasTable(string $table): bool
    {
        $cacheKey = '_schema_has_' . $table;
    ...
    // The courier module reads no core setting, so the host tells its config
    // screens whether the delivery partner service is switched on — the screens
    // stay reachable while it is off, but nothing configured there can be
    // assigned to an order until it is on.
