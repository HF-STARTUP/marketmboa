...
class OrderManager
{
    ...
    public static function getGroupWiseCouponArray($request, $couponCode, $applicableAmount, $discountAmount, $groupId, $cartListGroup): array
    {
        ...
        $coupon = $firstCoupon && $firstCoupon['coupon_type'] == 'first_order' ? $firstCoupon : (($firstCoupon && $firstCoupon['limit'] > $couponLimit) ? $firstCoupon : null);

        // Coupon expired / deactivated / usage-limit reached between the quote and
        // now (incl. a concurrent order crossing the limit at the same instant):
        // drop the discount instead of dereferencing a null coupon and throwing,
        // which surfaced as an opaque "could not place the order".
        if (!$coupon) {
            return [
                'discount' => 0,
                'coupon_bearer' => 'inhouse',
    ...
    public static function getVendorWiseCartList(array|object|null $data = []): array
    {
        ...
        foreach ($cartListQueryGroup as $groupId => $cartListQueryGroupItem) {
            ...
            $vendorWiseCartList[$groupId] = [
                ...
                'coupon_type' => $couponInfo['coupon_type'] ?? null,
    ...
    public static function processOrderGenerateData(array|object|null $data = []): array
    {
        ...
        foreach ($vendorWiseCartList as $vendorWiseGroupId => $vendorWiseCart) {
            ...
            } elseif ($vendorWiseCart['coupon_type'] == 'free_delivery') {
                $totalDiscountOnProduct = $vendorWiseCart['refer_and_earn_discount'];
            } else {
            ...
            foreach ($vendorWiseCart['cart_list'] as $cartItem) {
                if ($cartItem->product) {
                    ...
                    $taxableShippingCost = $vendorWiseCart['is_shipping_free'] == 1
                        ? $vendorWiseCart['shipping_cost'] - $vendorWiseCart['free_delivery_discount']
                        : ($vendorWiseCart['coupon_type'] == 'free_delivery' ? $vendorWiseCart['shipping_cost'] - $vendorWiseCart['coupon_discount'] : $vendorWiseCart['shipping_cost']);
                    $appliedShippingTaxAmount = CartManager::getAppliedShippingTaxAmount(
                        taxConfig: $taxConfig,
                        totalShippingCost: $taxableShippingCost,
                        totalDiscountedPrice: $totalDiscountPrice,
    ...
    public static function addOrderDetailsRewardData($orderId, $productId, $orderDetailsId, $price, $totalPrice, $couponCode, $couponDiscount): void
    {
        ...
        if ($loyaltyPointStatus && $loyaltyPointForEachOrder) {
            if (isset($businessSettings['loyalty_point_item_purchase_point']) && $businessSettings['loyalty_point_item_purchase_point'] > 0) {
                $couponDiscountShare = (!empty($couponCode) && $couponDiscount > 0 && $totalPrice > 0)
                    ? ((float)$couponDiscount * (float)$price) / $totalPrice
                    : 0;
                $loyaltyPointBase = Convert::default((float)$price - $couponDiscountShare);
                $individualLoyaltyPoint = floor($loyaltyPointBase * $businessSettings['loyalty_point_item_purchase_point'] / 100);
