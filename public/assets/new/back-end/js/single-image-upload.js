        ...
        if (resetBtn) {
            ...
            if (form) {
                form.querySelectorAll(".upload-file").forEach(card => {
                    // Reset = revert the form to its initial state, which for an
                    // edit page means showing the ORIGINAL saved image again — not
                    // clearing it. resetFileUpload() blanks the preview (that's the
                    // Remove-button behaviour), so use the revert helper here.
                    revertFileUploadToOriginal(card);
                });
            }
        }
    });
...
/**
 * Restore an upload card to its originally-saved image (used by the form Reset
 * button). checkPreExistingImages() stashes the saved src on the img's
 * `data-src`; revert to that so Reset keeps the existing image instead of
 * wiping it. Only when the card never had a saved image (add flow, or
 * `data-default-src` fallback) does it fall back to the empty placeholder.
 */
function revertFileUploadToOriginal(card) {
    const input = card.querySelector(".single_file_input");
    const imgElement = card.querySelector(".upload-file-img");
    const textbox = card.querySelector(".upload-file-textbox");
    const removeBtn = card.querySelector(".remove_btn");
    ...
    const originalSrc = imgElement?.dataset.src || imgElement?.dataset.defaultSrc || "";
