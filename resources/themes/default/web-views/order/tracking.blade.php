    ...
    <div class="container pt-4 pb-5 rtl">
        ...
        <div class="card border-0 box-shadow-lg">
            <div class="card-body py-5">
                ...
                @if(!empty($deliveryPartnerTracking))
                    <div class="border rounded bg-white p-3 mt-4">
                        <div class="row g-2">
                            @if($deliveryPartnerTracking['delivery_partner'])
                                <div class="col-sm-6 col-xl-4">
                                    <div class="media gap-3">
                                        <img alt="{{ translate('delivery_service_name') }}" width="20"
                                        ...
                                        <div class="media-body">
                                            ...
                                            <div class="font-weight-bold">{{ $deliveryPartnerTracking['delivery_partner'] }}</div>
                                        </div>
                                    </div>
                                </div>
                            @endif
                            @if($deliveryPartnerTracking['tracking_number'])
                                <div class="col-sm-6 col-xl-4">
                                    <div class="media gap-3">
                                        <img alt="{{ translate('tracking_ID') }}" width="20"
                                             src="{{ theme_asset(path: 'public/assets/front-end/img/icons/track_order.png') }}">
                                        <div class="media-body">
                                            <div class="text-muted">{{ translate('tracking_ID') }}</div>
                                            <div class="font-weight-bold">{{ $deliveryPartnerTracking['tracking_number'] }}</div>
                                        </div>
                                    </div>
                                </div>
                            @endif
                            @if($deliveryPartnerTracking['shipment_status'])
                                <div class="col-sm-6 col-xl-4">
                                    <div class="media gap-3">
                                        <img alt="{{ translate('Shipment_Status') }}" width="20"
                                             src="{{ theme_asset(path: 'public/assets/front-end/img/icons/track_order.png') }}">
                                        <div class="media-body">
                                            <div class="text-muted">{{ translate('Shipment_Status') }}</div>
                                            <div class="font-weight-bold text-capitalize">
                                                {{ translate($deliveryPartnerTracking['shipment_status_label']) }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="border-top mt-2 pt-2 fs-12 text-muted">
                            {{ $deliveryPartnerTracking['instruction'] }}
                        </div>
                    </div>
                @endif

                <div class="text-center pt-4 d-flex flex-wrap justify-content-center gap-2">
                    <a class="btn btn--primary btn-sm text-capitalize" href="#order-details"
                       data-toggle="modal">{{ translate('view_order_details')}}</a>
                    @if(!empty($deliveryPartnerTracking['tracking_url']))
                        <a class="btn btn-outline-primary btn-sm text-capitalize" href="{{ $deliveryPartnerTracking['tracking_url'] }}"
                           target="_blank" rel="noopener">{{ translate('track_with_delivery_partner') }}</a>
                    @endif
                </div>
            </div>
        </div>
