                                            ...
                                            @if($product->product_type == "physical")
                                                <div class="bg-white btn-circle border" style="--size: 35px"
                                                    ...
                                                    <img class="h-16px aspect-1 svg"
                                                        ...
                                                        alt="{{ translate('Physical_Product') }}">
                                                </div>
                                            @else
                                                <div class="bg-white btn-circle border" style="--size: 35px"
...
// Numéro WhatsApp pour le bouton principal
$whatsappPhone = $product->added_by == 'seller'
    ? ($product->seller->shop->contact ?? $product->seller->phone ?? null)
    : (getWebConfig(name: 'whatsapp')['phone'] ?? '');

if (empty($whatsappPhone)) {
    $whatsappPhone = getWebConfig(name: 'whatsapp')['phone'] ?? '';
}
                                                ?>
                                            ...
                                            <div class="product-add-and-buy-section gap-2 {!! $firstVariationQuantity <= 0 ? '' : 'd-flex' !!}"
                                                ...
                                                @else
                                                    <div
                                                        ...
                                                        <button type="button"
                                                            class="btn btn--primary element-center d-flex align-items-center justify-content-center gap-2 main-whatsapp-order-btn"
                                                            data-whatsapp="{{ $whatsappPhone }}"
                                                            data-form=".add-to-cart-details-form"
                                                            data-product-name="{{ $product->name }}">
                                                            <img src="{{ theme_asset(path: 'public/assets/front-end/img/icons/whatsapp.png') }}"
                                                                alt="{{ translate('whatsapp') }}" loading="eager"
                                                                style="width:1.1rem;height:auto;">
                                                            <span
                                                                class="string-limit whatsapp-btn-text">{{ translate('order_on_whatsapp') }}</span>
                                                        </button>
                                                        <button type="button"
                                        ...
                                        <div class="chat_with_seller-buttons">
                                            @if (auth('customer')->id())
                                                <button class="btn w-100 d-block text-center web--bg-primary text-white"
                                                    data-toggle="modal" data-target="#chatting_modal" @if(checkVendorAbility(type: 'vendor', status: 'temporary_close', vendor: $product->seller->shop)) disabled
                                                    @endif>
                                                    <img class="mb-1" alt=""
                            ...
                            <div class="row position-relative d-flex justify-content-between">
                                <div class="col-9">
                                    <a href="{{route('vendor-shop', ['slug' => getInHouseShopConfig(key: 'slug')]) }}"
                                        ...
                                        <div>
                                            ...
                                            <div class="fs-12 {{ session('direction') === "rtl" ? 'right' : 'ml-2' }}">
                                ...
                                <div class="col-12 position-static mt-3">
                                    <div class="chat_with_seller-buttons">
                                        @if (auth('customer')->id())
                                            <button class="btn w-100 d-block text-center web--bg-primary text-white"
                                                data-toggle="modal" data-target="#chatting_modal" {{ checkVendorAbility(type: 'inhouse', status: 'temporary_close') ? 'disabled' : '' }}>
                                                <img class="mb-1" alt=""
                                                    src="{{ theme_asset(path: 'public/assets/front-end/img/chat-16-filled-icon.png') }}">
                                                <span class="d-none d-sm-inline-block text-capitalize">
                                                    {{ translate('chat_with_vendor') }}
                ...
                <div class="pt-4 pb-3 d-flex justify-content-between align-items-center">
                    ...
                    @if($product->added_by == 'seller' && isset($product->seller->shop))
                        <a class="text-capitalize view-all-btn-text"
                            href="{{ route('vendor-shop', ['slug' => $product->seller->shop->slug]) }}">
...
@push('script')
    ...
    <script>
        $(document).ready(function () {
            ...
            // Script pour le bouton WhatsApp principal
            $(document).on('click', '.main-whatsapp-order-btn', function (e) {
                e.preventDefault();

                const button = $(this);
                let whatsapp = button.data('whatsapp') || '';
                if (!whatsapp) {
                ...
                const totalPrice = form.find('.product-details-chosen-price-amount').text().trim() || '';

                const selectedColorInput = form.find('input[name="color"]:checked');
                const selectedColorLabel = selectedColorInput.length
                    ? form.find(`label[for="${selectedColorInput.attr('id')}"]`)
                    : null;
                const selectedColorName = selectedColorLabel ? selectedColorLabel.data('title') || '' : '';

                const choiceParts = [];
                form.find('.checkbox-alphanumeric input[type="radio"]:checked').each(function () {
                    const input = $(this);
                    if (input.attr('name') === 'variant_key') return;
                    const label = form.find(`label[for="${input.attr('id')}"]`);
                    const value = label.length ? label.text().trim() : input.val();
                    choiceParts.push(`${input.attr('name')} : ${value}`);
                });

                const selectedExtension = form.find('input[name="variant_key"]:checked');
                const extensionLabel = selectedExtension.length
                    ? form.find(`label[for="${selectedExtension.attr('id')}"]`).text().trim()
                    : '';
                ...
                if (totalPrice) message += `  Total : ${totalPrice}\n`;
    ...
    <style>
        /* Styles pour le bouton WhatsApp principal */
        .main-whatsapp-order-btn {
        ...
        .main-whatsapp-order-btn:hover {
            background-color: #1da851 !important;
            border-color: #1da851 !important;
            color: #fff !important;
        }

        .main-whatsapp-order-btn img {
            filter: brightness(0) invert(1);
        }

        /* Responsive pour le bouton WhatsApp principal */
        @media (max-width: 767.98px) {
            ...
            .main-whatsapp-order-btn .whatsapp-btn-text {
                display: none;
            }
