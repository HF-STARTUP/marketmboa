...
function updateNavCart(retryOnFailure = true) {
    ...
    $.post(
        ...
        // A dropped refresh leaves a stale count and dropdown with no sign anything went wrong.
        // A transient failure is worth one retry; an expired CSRF token (419) will never recover.
        if (retryOnFailure && xhr.status !== 419) {
            setTimeout(function () {
                updateNavCart(false);
            }, 1000);
        }
