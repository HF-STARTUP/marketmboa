<?php
$formattedCart = collect($productData)->map(function ($itemKey, $itemValue) use ($productData) {
    return [
...
<form action="{{ route('cart.add') }}" method="POST">
    ...
    <div class="d-flex flex-column flex-sm-row justify-content-center gap-2 mt-4">
        ...
        <a href="javascript:void(0)"
            class="whatsapp-order-btn btn btn--primary font-weight-semibold rounded-10 py-2 text-capitalize d-flex align-items-center justify-content-center gap-2 flex-grow-1"
            data-whatsapp="{{ $whatsappPhone }}" data-cart='@json($formattedCart)'
            data-subtotal="{{ $totalAmountFormatted }}">
            <img src="{{ theme_asset('assets/img/icons/whatsapp.svg') }}" alt="{{ translate('cart') }}" loading="eager"
