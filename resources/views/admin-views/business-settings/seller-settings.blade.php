...
@section('content')
    <div class="content container-fluid">
        ...
        <form action="{{ route('admin.business-settings.vendor-settings.update-vendor-settings') }}" method="post" id="update-vendor-settings-form">
            ...
            @if(function_exists('getCheckAddonPublishedStatus') && getCheckAddonPublishedStatus(moduleName: 'Builder'))
                <div class="card mb-3">
                    <div class="card-body">
                        <div class="p-12 p-sm-20 bg-section rounded">
                            <div class="d-flex justify-content-between align-items-center gap-3">
                                <div>
                                    <h2>{{ translate('vendor_website_builder') }}</h2>
                                    <p class="mb-0">
                                        {{ translate('enable_this_option_to_allow_vendors_to_set_up_and_manage_their_own_website.') }}
                                    </p>
                                </div>
                                <div>
                                    @php($adminWebsiteBuilderStatus = getWebConfig('admin_website_builder_status') ?? 0)
                                    <label class="switcher" for="admin-website-builder-status">
                                        <input class="switcher_input" type="checkbox" value="1"
                                               name="admin_website_builder_status" id="admin-website-builder-status"
                                               {{ $adminWebsiteBuilderStatus == 1 ? 'checked' : '' }}
                                               data-modal-type="input-change"
                                               data-on-title="{{ translate('want_to_enable_vendor_website_setup') . '?' }}"
                                               data-off-title="{{ translate('want_to_disable_vendor_website_setup') . '?' }}"
                                               data-on-message="<p>{{ translate('if_enabled,_vendors_will_have_the_freedom_to_create,_edit,_and_manage_their_own_websites_independently.') }}</p>"
                                               data-off-message="<p>{{ translate('if_disabled,_vendors_will_not_be_able_to_create_or_manage_their_own_websites.') }}</p>"
                                               data-on-button-text="{{ translate('turn_on') }}"
                                               data-off-button-text="{{ translate('turn_off') }}">
                                        <span class="switcher_control"></span>
                                    </label>
            ...
            <div class="d-flex justify-content-end trans3 mt-4 action-btn-wrapper-container">
                <div class="d-flex justify-content-sm-end justify-content-center gap-3 flex-grow-1 flex-grow-sm-0 bg-white action-btn-wrapper trans3">
                    ...
                    <button type="{{ getDemoModeFormButton(type: 'button') }}" class="btn btn-primary px-3 px-sm-4 {{ getDemoModeFormButton(type: 'class') }}">
