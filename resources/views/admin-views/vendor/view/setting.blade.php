...
@section('content')
    ...
    <div class="content container-fluid">
        ...
        <div class="row g-3">
            ...
            @if ($builderPublished ?? false)
                <div class="col-md-6">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between gap-2">
                            <h4 class="mb-0 d-flex gap-2">
                                {{ translate('builder_Storefront') }}

                                @if (!empty($builderConfig->domain))
                                    <small class="text-muted d-block">
                                        ({{ translate('domain') }}: <a href="{{ (request()->secure() ? 'https://' : 'http://').$builderConfig->domain }}" target="_blank">{{ $builderConfig->domain }})</a>
                                    </small>
                                @endif
                            </h4>
                        </div>
                        <div class="card-body">
                            <form action="{{ route('admin.vendors.update-setting',['id'=>$seller['id']]) }}" method="post" id="update-setting-form-3">
                                @csrf
                                <input type="hidden" name="builder_status_update" value="1">
                                <div class="form-group">
                                    <div class="d-flex justify-content-between align-items-center gap-10 form-control">
                                            <span class="title-color text-capitalize">
                                                {{ translate('builder_storefront_access') }}
                                                <span class="input-label-secondary cursor-pointer" data-bs-toggle="tooltip" data-bs-placement="right" title="{{ translate('if_enabled_the_vendor_can_open_the_website_builder_and_configure_their_custom_website') }}">
                                                    <img width="16" src="{{dynamicAsset(path: 'public/assets/back-end/img/info-circle.svg')}}" alt="">
                                                </span>
                                            </span>
                                        <label class="switcher" for="builder-status">
                                            <input
                                                class="switcher_input custom-modal-plugin"
                                                type="checkbox" value="1" name="builder_status"
                                                id="builder-status"
                                                {{ $builderConfig?->website_visibility ? 'checked':'' }}
                                                data-modal-type="input-change"
                                                data-modal-form="#update-setting-form-3"
                                                data-on-image="{{ dynamicAsset(path: 'public/assets/new/back-end/img/modal/general-icon.png') }}"
                                                data-off-image="{{ dynamicAsset(path: 'public/assets/new/back-end/img/modal/general-icon.png') }}"
                                                data-on-title="{{translate('want_to_Turn_ON_Builder_Storefront_For_This_Vendor')}}"
                                                data-off-title="{{translate('want_to_Turn_OFF_Builder_Storefront_For_This_Vendor')}}"
                                                data-on-message="<p>{{translate('if_enabled_the_vendor_can_open_the_website_builder_and_configure_and_show_their_custom_website')}}</p>"
                                                data-off-message="<p>{{translate('if_disabled_the_vendor_cannot_access_the_website_builder_or_show_their_custom_website')}}</p>"
                                                data-on-button-text="{{ translate('turn_on') }}"
                                                data-off-button-text="{{ translate('turn_off') }}">
                                            <span class="switcher_control"></span>
                                        </label>
