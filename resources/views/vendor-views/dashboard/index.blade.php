...
@section('content')
    <div class="content container-fluid">
        ...
        @php($adminWebsiteBuilderStatus = getWebConfig('admin_website_builder_status') ?? 0)
        ...
        {{-- Query the table directly, never the Builder model class: once the
            ...
            classmap still references it, so class_exists()/the model would try
            to include a missing file and fatal. Schema::hasTable stays safe. --}}
        @php($vendorBuilderConfig = \Illuminate\Support\Facades\Schema::hasTable('tenant_domain_configs')
            ? \Illuminate\Support\Facades\DB::table('tenant_domain_configs')->where('tenant_id', auth('seller')->id())->first()
            : null)
        @php($vendorBuilderVisible = $vendorBuilderConfig && $vendorBuilderConfig->website_visibility)

        @if(function_exists('getCheckAddonPublishedStatus') && getCheckAddonPublishedStatus(moduleName: 'Builder') && $adminWebsiteBuilderStatus && $vendorBuilderVisible)
            <div class="card mb-3 border-0 overflow-hidden website-builder-banner">
                <div class="card-body p-4">
                    <div class="row align-items-center g-3">
                        <div class="col-lg-8 col-md-7">
                            <div class="d-flex align-items-center gap-3">
                                <span class="website-builder-banner__icon">
                                    <i class="fi fi-sr-palette"></i>
                                </span>
                                <div>
                                    <h4 class="mb-1 text-white font-weight-bold">
                                        {{ translate('Build_Your_Custom_Website') }}
                                    </h4>
                                    <p class="mb-0 text-white-50">
                                        {{ translate('design,_customize,_and_launch_your_own_store_website_without_writing_a_single_line_of_code') }}.
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-4 col-md-5 text-md-right">
                            <a href="{{ route('vendor.builder.index', ['page' => 'global-settings']) }}"
                               class="btn btn-light font-weight-medium text-nowrap" id="vendor-dashboard-builder-button">
                                <i class="tio-edit mr-1"></i>{{ translate('open_website_builder') }}
                            </a>
                        </div>
                    </div>
                </div>
