...
@section('content')
    ...
    @if($order['order_status'] != 'delivered')
        ...
        @if(!empty($courierPartnerAssigned))
            <div class="modal fade" id="courierAddressChangeModal" tabindex="-1" role="dialog"
                 aria-labelledby="courierAddressChangeModalLabel">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="modal-header pb-0">
                            <h5 class="modal-title font-weight-bold"
                                id="courierAddressChangeModalLabel">{{translate('Update_the_delivery_address')}}?</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <p class="mb-3">{{translate('This_order_has_already_been_assigned_to_a_delivery_partner_Changing_the_address_here_does_not_automatically_update_the_address_with_the_delivery_partner')}}</p>
                            <div class="alert alert-warning fs-12 mb-0">
                                {{translate('Update_the_address_through_your_merchant_or_provider_account_or_contact_the_delivery_partner_so_the_shipment_information_is_corrected')}}
                            </div>
                        </div>
                        <div class="modal-footer border-0 d-flex gap-3">
                            ...
                            <button type="button" class="btn btn-primary flex-fill"
                                    id="courier-address-change-confirm">{{translate('Confirm_and_Update')}}</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <script>
                (function () {
                    const form = document.getElementById('shippingAddressUpdateForm');
                    const confirmBtn = document.getElementById('courier-address-change-confirm');
                    if (!form || !confirmBtn || !window.jQuery) return;

                    form.addEventListener('submit', function (event) {
                        if (!form.reportValidity()) return;
                        event.preventDefault();
                        $('#courierAddressChangeModal').modal('show');
                    });

                    confirmBtn.addEventListener('click', function () {
                        $('#courierAddressChangeModal').modal('hide');
                        form.submit();
                    });
                })();
            </script>
