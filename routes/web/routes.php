...
// Preview-locale switch for the vendor-panel Builder. Its LanguageSwitcher
// navigates a full page to /lang/{locale}; without this route the request fell
// through to the update.php fallback (redirect to '/'), kicking the vendor out
// of the editor and never changing the language. Set `landing_local` — the same
// session key ShareBuilderProps exposes as `currentLocale` — and return to the
// builder. Only accept a configured language code; ignore anything else.
Route::get('lang/{locale}', function (string $locale) {
    $isKnown = collect(getWebConfig(name: 'language') ?? [])->contains('code', $locale);
    if ($isKnown) {
        session(['landing_local' => $locale]);
    }
    return redirect()->back();
})->name('builder.set-landing-locale');
