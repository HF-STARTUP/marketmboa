...
<?php ($cartList = \App\Utils\CartManager::getCartListQuery()); ?>
<?php ($sub_total = 0); ?>
<?php ($total_tax = 0); ?>
...
<div class="navbar-tool dropdown me-2 <?php echo e(Session::get('direction') === "rtl" ? 'mr-md-3' : 'ml-md-3'); ?>">
    <?php if($web_config['guest_checkout_status'] || auth('customer')->check()): ?>
        ...
        <a class="navbar-tool-text ms-2" href="<?php echo e(route('shop-cart')); ?>"><small><?php echo e(translate('my_cart')); ?></small>
    ...
    <?php else: ?>
        <a class="navbar-tool-icon-box bg-secondary dropdown-toggle" href="<?php echo e(route('customer.auth.login')); ?>">
            <span class="navbar-tool-label text-white bg-danger">
                <?php echo e($cart->count()); ?>

            </span>
            <i class="navbar-tool-icon czi-cart"></i>
        </a>
        <a class="navbar-tool-text ms-2" href="<?php echo e(route('customer.auth.login')); ?>">
            ...
            <?php if($cart->count() > 0): ?>
                    ...
                    <?php $__currentLoopData = $cart; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cartItem): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <div class="widget-cart-item">
                            <div class="media">
                                ...
                                <div class="media-body min-height-0 d-flex align-items-center <?php echo e($product ? ($product->status == 0 ? 'blur-section' : '') : 'blur-section'); ?>">
                                    <div class="w-0 flex-grow-1">
                                        ...
                                        <div class="widget-product-meta">
                                            <span class="text-muted me-2">x <span class="cart_quantity_multiply<?php echo e($cartItem['id']); ?>"><?php echo e($cartItem['quantity']); ?></span></span>
                ...
                <?php if($free_delivery_status['status'] && (session()->missing('coupon_type') || session('coupon_type') != 'free_delivery')): ?>
                    <div class="py-3">
                        ...
                        <small class="amount_need_to_fullfill <?php echo e($free_delivery_status['amount_need'] <= 0 ? 'd-none' : ''); ?>">
                            <span class="text-accent __text-12px free_delivery_amount_need"><?php echo e(webCurrencyConverter(amount: $free_delivery_status['amount_need'])); ?></span>
                            <?php echo e(translate('add_more_for_free_delivery')); ?>
                ...
                <div class="d-flex flex-wrap justify-content-between align-items-center pb-2">
                    <div class="font-size-sm <?php echo e(Session::get('direction') === "rtl" ? 'ml-2 float-left' : 'mr-2 float-right'); ?> py-2">
                        <span><?php echo e(translate('subtotal')); ?> :</span>
                        <span class="text-accent font-size-base cart_total_amount <?php echo e(Session::get('direction') === "rtl" ? 'mr-1' : 'ml-1'); ?>">
                    ...
                    <?php if($web_config['guest_checkout_status'] || auth('customer')->check()): ?>
                        <a class="btn btn-outline-secondary btn-sm" href="<?php echo e(route('shop-cart')); ?>">
                            <?php echo e(translate('expand_cart')); ?><i class="czi-arrow-<?php echo e(Session::get('direction') === "rtl" ? 'left mr-1 ml-n1' : 'right ml-1 mr-n1'); ?>"></i>
                        </a>
                    <?php else: ?>
                ...
                <?php ($sellerIds = $cart->pluck('seller_id')->unique()); ?>
                <?php if($cart->count() > 0): ?>
                    <?php if($sellerIds->count() == 1): ?>
                        <?php ($sellerModel = \App\Models\Seller::find($sellerIds[0])); ?>
                        <?php ($targetPhoneDropdown = $sellerModel ? (($sellerModel->country_code ?? '') . ($sellerModel->phone ?? '')) : ($whatsapp['phone'] ?? '237696472943')); ?>
                    <?php else: ?>
                        <?php ($targetPhoneDropdown = $whatsapp['phone'] ?? '237696472943'); ?>
                    <?php endif; ?>
                    <button type="button"
                        class="btn btn-success btn-sm btn-block font-bold rounded text-capitalize mb-2 cart-whatsapp-order-btn"
                        data-whatsapp="<?php echo e($targetPhoneDropdown); ?>"
                        style="background-color:#25D366; color:#fff; border:none; display:flex; align-items:center; justify-content:center; gap:8px;">
                        <img src="<?php echo e(theme_asset('public/assets/front-end/img/whatsapp.svg')); ?>" width="18" height="18" alt="WhatsApp" style="filter: brightness(0) invert(1);">
                        <?php echo e(translate('order_on_whatsapp')); ?>
    ...
    <?php
        $cartItemsForWhatsapp = $cart->map(function ($ci) {
            return [
                'name' => $ci['name'],
                'qty' => (int) $ci['quantity'],
                'unit_price' => (int) $ci['price'],
                'total' => (int) ($ci['price'] * $ci['quantity']),
            ];
        })->values()->all();
    ?>

    <script>
        document.querySelector('.cart-whatsapp-order-btn')?.addEventListener('click', function() {
            var phone = this.dataset.whatsapp || '237696472943';
            var items = <?php echo json_encode($cartItemsForWhatsapp, 15, 512) ?>;
            var lines = items.map(function(i) {
                return i.name + ' (' + i.qty + 'x ' + i.unit_price + ' = ' + i.total + ')';
            });

            var subtotal = '<?php echo e(webCurrencyConverter(amount: $sub_total)); ?>';
            var shopUrl = '<?php echo e(url('/')); ?>';

            var msg = '🛒 *Commande complète*\n\n'
                    + '📦 Articles :\n' + lines.join('\n') + '\n\n'
                    + '💰 Total : ' + subtotal + '\n'
                    + '🔗 ' + shopUrl;
...
</div><?php /**PATH /opt/projects/Genius/MarketMboa/code/resources/themes/default/layouts/front-end/partials/_cart.blade.php ENDPATH**/ ?>